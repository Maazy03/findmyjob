...
const GridTables = (props) => {
  ...
  useEffect(() => {
    // console.log("no i am not called");
    setStateForArts(reduxArtsState.generalArts);
  });
  ...
  async function getData(area) {
    ...
    let get = await dispatch(productActions.getGeneralArts(index));
    // console.log("check new data", get);
    setStateForArts(get);
    ...
    return (
      <tr key={arts._id} style={{ cursor: "pointer" }}>
        <SweetAlert
          ...
          showCancelButton
          onConfirm={(e) => onConfirmDeleteProduct()}
          onCancel={() =>
            setStateSweetAlert((prevState) => ({
              ...prevState,
              message5: false,
      ...
      >
        {laoding ? (
          ...
          <Fragment>
            ...
            <TablePagination tableName={"general"} area={region} />
          </Fragment>
        )}
      </ReactCSSTransitionGroup>
    </Fragment>
